...
export function prepareLayout (nodes, hmargin, layers, nodeMap) {
  var maxLayer = 0
...
export function respaceGraph (nodes, edges) {
...
export function spreadLayerSmart (layer) {
  var ranges = []
  ranges.push({
    start: 0,
    end: 0,
...
export function spaceVertically (layers, maxLayer) {
  var startY = 0
...
export function uncrossWithIn (layer) {
  for (var i = 0; i < layer.length; ++i) {
...
export function findAverage (nodes) {
  var sum = 0
  for (var i = 0; i < nodes.length; ++i) {
    sum += nodes[i].x
  }
...
export function uncrossWithOut (layer) {
  for (var i = 0; i < layer.length; ++i) {
